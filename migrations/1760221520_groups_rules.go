...
func init() {
	m.Register(func(app core.App) error {
		...
		if err := json.Unmarshal([]byte(`{
			"listRule": "@request.auth.id != \"\" && (teacher = @request.auth.id || students.id ?= @request.auth.id)",
			"viewRule": "@request.auth.id != \"\" && (teacher = @request.auth.id || students.id ?= @request.auth.id)",
