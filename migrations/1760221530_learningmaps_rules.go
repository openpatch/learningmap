...
func init() {
	m.Register(func(app core.App) error {
		...
		// Teachers can CRUD their own learning maps
		if err := json.Unmarshal([]byte(`{
			"listRule": "@request.auth.id != \"\" && @request.auth.role = \"teacher\" && teacher = @request.auth.id",
			"viewRule": "@request.auth.id != \"\" && @request.auth.role = \"teacher\" && teacher = @request.auth.id",
