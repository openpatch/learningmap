    ...
    const loadMap = async () => {
      ...
      if (existingMap) {
        ...
        if (storageId && storageId !== jsonId) {
          ...
          if (
            mapWithStorageId &&
            mapWithStorageId.roadmapData !== existingMap.roadmapData
          ) {
            ...
            if (shouldReplace) {
              ...
              await db.addLearningMap(
                storageId,
                existingMap.roadmapData,
                existingState,
              );
      ...
      if (jsonId.includes(".") || jsonId.includes("/")) {
        setError(
          "Learning map not found. It may not have been loaded yet. Please make sure you have accessed the original map first.",
        );
        return;
      }
    ...
    <div className="learn-list-container">
      ...
      <div className="learn-list-content">
        <div className="page-header">
          <h2>
            <span className="page-emoji">👨‍🎓</span> My Learning Maps
          </h2>
          ...
          <div className="maps-grid">
            {allMaps.map((map) => {
              const completed = Object.values(map.state?.nodes || {}).filter(
                (node: any) =>
                  node.state === "completed" || node.state === "mastered",
              ).length;
